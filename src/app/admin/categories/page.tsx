...
function normalizeAttributeSchema(schema: any): any[] {
  let schemaArray: any[] = []
  if (typeof schema === "string") {
    try {
      const cleanSchema = schema.trim().replace(/'/g, '"')
      schemaArray = JSON.parse(cleanSchema)
    } catch (e) {
      try {
        schemaArray = JSON.parse(schema)
      } catch (err) {
        return []
      }
    }
  ...
  return schemaArray.map((attr: any) => {
    const label = attr.label || attr.name || ""
    const key = attr.key || label.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/(^-|-$)+/g, "")
    const type = attr.type || "select"
    const required = attr.required ?? false
    
    let options = attr.options
    if (!Array.isArray(options)) {
      if (type === "boolean" || String(type).toLowerCase() === "boolean") {
        options = ["Yes", "No"]
      } else {
        options = []
      }
...
export default function AdminCategoriesPage() {
  ...
  const startEditingCategory = (category: Category) => {
    ...
    const schemaArray = normalizeAttributeSchema(category.attribute_schema)
  ...
  const addVisualAttributeRow = () => {
    setVisualAttributes((prev) => [
      ...prev,
      { key: "", label: "", type: "select", optionsString: "", required: false }
    ])
  }
  ...
  const updateVisualAttribute = (index: number, field: keyof VisualAttribute, value: any) => {
    setVisualAttributes((prev) => {
      const updated = [...prev]
      updated[index] = {
        ...updated[index],
    ...
    if (schemaMode === "json") {
      ...
      const compiled = visualAttributes
        ...
        .map((attr) => ({
          ...
          options: attr.type === "select" || attr.type === "boolean"
            ? attr.optionsString.split(",").map((o) => o.trim()).filter(Boolean)
            : [],
        }))
      setRawJsonSchema(JSON.stringify(compiled, null, 2))
    } else {
      // Decompile JSON string to visual attributes
      const normalized = normalizeAttributeSchema(rawJsonSchema)
      const decompiled = normalized.map((attr: any) => ({
        key: attr.key,
        label: attr.label,
        type: attr.type || "select",
        optionsString: Array.isArray(attr.options) ? attr.options.join(", ") : "",
                      ...
                      {visualAttributes.map((attr, index) => (
                        <div key={index} className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-start p-4 bg-slate-50/50 border border-slate-100 rounded-2xl">
                          ...
                          <div className="sm:col-span-2 flex flex-col gap-1 text-left">
                            ...
                            <select
                              value={attr.type}
                              onChange={(e) => updateVisualAttribute(index, "type", e.target.value)}
                              className="w-full rounded-lg border border-outline-variant/30 bg-white px-3 py-2 text-xs font-semibold outline-none focus:border-primary"
                            >
                              <option value="select">Dropdown</option>
                              <option value="text">Text Input</option>
                              <option value="number">Number Input</option>
                              <option value="boolean">Yes/No Select</option>
                            </select>
                          </div>

                          <div className="sm:col-span-3 flex flex-col gap-1 text-left">
                            <span className="text-[10px] font-bold uppercase text-slate-500">Options</span>
                            <input
                              type="text"
                              required={attr.type === "select"}
                              disabled={attr.type !== "select"}
                              value={attr.type === "select" ? attr.optionsString : attr.type === "boolean" ? "Yes, No" : ""}
                              onChange={(e) => updateVisualAttribute(index, "optionsString", e.target.value)}
                              placeholder={attr.type === "select" ? "e.g. 4GB, 8GB" : "N/A"}
                              className="w-full rounded-lg border border-outline-variant/30 disabled:bg-slate-100 disabled:text-slate-400 bg-white px-3 py-2 text-xs font-semibold outline-none focus:border-primary"
                            />
                          </div>

                          <div className="sm:col-span-1 pt-5 self-center flex flex-col items-center justify-center gap-1">
                            <span className="text-[9px] font-bold uppercase text-slate-500">Req.</span>
                            <input
                              type="checkbox"
                              checked={attr.required}
                              onChange={(e) => updateVisualAttribute(index, "required", e.target.checked)}
                              className="size-4 rounded border-outline-variant/30 text-primary focus:ring-primary accent-[var(--seller-primary)] cursor-pointer"
                            />
                          </div>
